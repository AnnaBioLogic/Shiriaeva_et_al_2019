...
setwd("D:/Genomic_DNA_ss/403_plusI")





#In this run we had 1x150 reads but to make analysis identical to analysis of reads obtained with NEBNextUltraII kit we will split 1x150 reads into 2 by 75 reads


reads_F<-readFastq("KD403_WithCasInduction_GenomicDNA_Accel_NGS_1S_Plus.fastq")

log<-width(reads_F)>=75
reads_long<-reads_F[log]
reads_short<-reads_F[!log]
reads_long1<-narrow(reads_long, 1, 75)
reads_long2<-narrow(reads_long, 76, width(reads_long))


reads_F<-append(reads_short, reads_long1)
reads_F<-append(reads_F,  reads_long2)
...
hits_3prime=vcountPattern("GATCGGAAGAGCAC",reads,
...
if(length(M1_R1_1)>=1){
start_adapter<-startIndex(vmatchPattern("GATCGGAAGAGCAC",M1_R1_1,max.mismatch=1, with.indels=FALSE))
...
bamFile<-BamFile("bam_42_all.bam")
p_pl<-ScanBamParam(what=scanBamWhat())


filter <- FilterRules(list(strand = function(x) (x$strand) == "+"))
destination<-"plus.bam"
dest <- filterBam(bamFile, destination, param=p_pl, filter=filter)
countBam(dest) 



filter <- FilterRules(list(strand = function(x) (x$strand) == "-"))
destination<-"minus.bam"
dest <- filterBam(bamFile, destination, param=p_pl, filter=filter)
countBam(dest) 
...
##Whole genome plot
plot(UNCOV1_vector, type="h", xlim=c(1, width(mychr)), ylim=c(0, 300), xlab="bp", ylab="Coverage", main=name )

#################plot PPS-/+300kb

plot(UNCOV1_vector, type="h", xlim=c(275539, 875570), ylim=c(0, 300), xlab="bp", ylab="Coverage", main=name )
...
###You can find following values in Supplementary Table 2:
...
alignsTest1<-readGAlignments('plus.bam')
length(alignsTest1) #(this is is how many reads aligned you have)
COV1<-coverage(alignsTest1)
UNCOV1<-unlist(COV1$KD403)
UNCOV1_L<-runLength(UNCOV1)
...
write.table(df,file='DIR_genomic_cov_per_1kb')
...
alignsTest1<-readGAlignments('minus.bam')
length(alignsTest1) #(this is is how many reads aligned you have)
COV1<-coverage(alignsTest1)
UNCOV1<-unlist(COV1$KD403)
UNCOV1_L<-runLength(UNCOV1)
...
coverage_normalized_percent<-100*(as.numeric(as.character(df$`average coverage`)))/sum
df<-cbind(df, coverage_normalized_percent)
write.table(df,file='Rev_genomic_cov_per_1kb')
